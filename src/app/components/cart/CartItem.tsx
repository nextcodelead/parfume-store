...
export const CartItem: React.FC<CartItemProps> = ({ 
  ...
  // Derive all values from props - no local state needed
  const selectedStock = cart.stock ?? null;
  const quantity = cart.count;
  
  // Computed values
  const haveInStock = cart.product.stocksCount > 0 && (selectedStock?.quantity ?? 0) > 0;
  const unitPrice = selectedStock ? (selectedStock.discount ?? selectedStock.cost) : 0;
  const subtotal = unitPrice * quantity;
  const imageUrl = cart.product.photo?.imageUrl 
    ? `https://dataset.uz/${cart.product.photo.imageUrl}` 
    : "https://placehold.jp/3d4070/ffffff/150x150.png?text=No%20image";
  const brandName = cart.product.brand?.name ?? cart.product.name ?? "Товар";

  const handleQuantityChange = (newQuantity: number) => {
    if (newQuantity >= 1) {
      onUpdateQuantity(cart.pk, newQuantity);
    }
  };

  const handleStockSelect = (stock: Stock) => {
    onSetStock(cart.pk, stock);
  };
  ...
  return (
    <div 
      className={`bg-white rounded-lg p-3 sm:p-4 shadow-sm hover:shadow-md transition-shadow flex flex-col sm:flex-row gap-3 sm:gap-4 ${
        !haveInStock ? "opacity-60" : ""
      }`}
    >
      {/* Image */}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Header with brand, name and remove button */}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mt-auto">
          ...
          <div className="flex items-center border border-gray-300 rounded-lg self-start sm:self-auto">
            ...
            <span className="px-3 sm:px-4 font-semibold text-sm sm:text-base min-w-[36px] sm:min-w-[44px] text-center">
              {quantity}
            </span>
            <button
              onClick={() => handleQuantityChange(quantity + 1)}
