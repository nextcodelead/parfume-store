...
const ProductInfo: React.FC<Props> = ({ productId }) => {
  ...
  const [isFavorite, setIsFavorite] = useState(isInWishlist(productId));
  ...
  if (error) {
    return (
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="text-red-600 text-sm sm:text-base">Ошибка загрузки информации о товаре: {error.message}</div>
      </div>
    );
  }
  if (loading || !productData) {
    return (
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="text-gray-600 text-sm sm:text-base">Загрузка информации о товаре...</div>
      </div>
    );
  }
  return (
        ...
        <div className="order-2 lg:order-2 space-y-4 sm:space-y-5 lg:space-y-6">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3 text-xs sm:text-sm text-gray-600">
              {selectedSize && selectedSize.quantity > 0 ? (
                <span className="inline-flex items-center gap-1.5 sm:gap-2 text-green-600 font-medium">
                  <Check size={14} className="sm:w-4 sm:h-4" /> <span>В наличии</span>
                </span>
              ) : (
                <span className="text-red-600 font-semibold">Нет в наличии</span>
              )}
              {selectedSize?.article && (
                <span className="text-gray-500 text-xs sm:text-sm">артикул: {selectedSize.article}</span>
              )}
            </div>
          </div>
          ...
          {productData.product.aromNote && (
            <div className="bg-rose-50 rounded-lg p-3 sm:p-4 lg:p-5">
              <h3 className="font-semibold text-gray-900 mb-2 sm:mb-3 text-sm sm:text-base">Ароматные ноты</h3>
          ...
          {productData.product.stocks.length > 0 && (
            <div>
              <label className="block text-sm sm:text-base font-semibold text-gray-900 mb-2 sm:mb-3">Размер</label>
          ...
          {/* Quantity */}
          <div>
            <label className="block text-sm sm:text-base font-semibold text-gray-900 mb-2">Количество</label>
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 pt-2">
            ...
            >
              {selectedSize && selectedSize.quantity > 0 ? 'Добавить в корзину' : 'Нет в наличии'}
            </Button>
              ...
              <div className="relative">
                ...
                {showShareMenu && (
                  <div className="absolute right-0 sm:right-auto sm:left-0 mt-2 bg-white rounded-lg shadow-xl p-2 z-10 space-y-1 min-w-[140px] sm:min-w-[160px] border">
                    ...
                    <button onClick={() => handleShare('copy')} className="block w-full text-left px-3 py-2 hover:bg-gray-100 rounded text-xs sm:text-sm transition-colors">Копировать ссылку</button>
                  </div>
                )}
              </div>
            </div> */}
